...
Drawing bullet graphs using `R` is extremely easy by means of the `bulletgraph()`. The function is described on the project wiki: [https://github.com/mtorchiano/MTkR/wiki/Bullet-Graph]. The code has been released as open-source.

This documents reproduces the original [Bullet Graph Design Specification](http://www.perceptualedge.com/articles/misc/Bullet_Graph_Design_Spec.pdf) by Stephen Few (pending permission).

In addition it shows the `R` code that is used to generate the graphs (omitting a few embellishments and decorations). The code makes use of the `bulletgraph()` function.

The goal of this document is to show that the function is able to reproduce all the examples provided in the original specification document. The only exception being (at the time of this writing) vertical graph: currently the function is able to draw only horizontal bullet graphs.
...
```{r, fig.show='hold',echo=TRUE,fig.width=7,fig.height=5}
par(mfrow=c(5,1))
par(mar=c(3,11,1,1))
bulletgraph(x=270,ref=260,limits=c(0,200,250,300),
            name= "Revenue",subname="U.S. $ (1,000s)",cex.names=1.5)
bulletgraph(x=.225,ref=.27,limits=c(0,.2,.25,.3),
            name= "Profit",subname="%",cex.names=1.5)
bulletgraph(x=325,ref=550,limits=c(0,350,500,600),
            name= "Avg Order Size",subname="U.S. $",cex.names=1.5)
bulletgraph(x=1700,ref=2050,limits=c(0,1450,2000,2500),
            name= "New Customers",subname="Count",cex.names=1.5)
bulletgraph(x=4.7,ref=4.5,limits=c(0,3.5,4.2,5),
            name= "Cust Satisfaction",subname="Top Rating of 5",cex.names=1.5)
```
...
```{r, fig.show='hold',echo=TRUE,fig.width=6,fig.height=1.3}
par(mar=c(3,9,1,1))
